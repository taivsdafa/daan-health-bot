...
historical_logs = []  # 記憶體中的歷史紀錄

def send_line_push(message_text, user_id, status_desc):
    ...
    try:
        ...
        # 寫入歷史日誌
        loc = user_locations.get(user_id, "未填入")
...
# 📊 網頁版日誌下載端 (護理師隨時輸入網址就能下載，不用關閉伺服器！)
@app.route("/download_log", methods=['GET'])
def download_log():
    if not historical_logs:
        # 如果空空的，自動給一筆範例
        display_logs = [{"通報時間": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "使用者識別碼": "User_Sample", "通報事件位置": "測試位置", "傷勢嚴重分流": "伺服器正常上線中"}]
    else:
...
def index():
    return f"<h3>大安高工健康小幫手雲端伺服器運行中！</h3>目前記憶庫已累積 {len(historical_logs)} 筆紀錄。<br><br><a href='/download_log'><button>點我下載今日 Excel 日誌</button></a>", 200
